            ...
            <div class="wrapper">
                <div class="table-box">
                    <div class="row">
                        ...
                        $total_pages_sql = "SELECT COUNT(*) FROM borrow  ";
                        $result = mysqli_query($conn, $total_pages_sql);
                        $total_rows = mysqli_fetch_array($result)[0];
                        $total_pages = ceil($total_rows / $no_of_records_per_page);
                        $sql = " SELECT * FROM borrow INNER JOIN book ON borrow.bookID = book.bookID INNER JOIN borrowstatus ON borrow.statusID = borrowstatus.bstatusID INNER JOIN member ON borrow.memberID = member.memberID WHERE (bookName LIKE '%" . $text_keyword . "%' or author LIKE '%" . $text_keyword . "%' or Firstname LIKE '%" . $text_keyword . "%' or Lastname LIKE '%" . $text_keyword . "%') LIMIT $offset, $no_of_records_per_page ";
                        $res_data = mysqli_query($conn, $sql);
                        echo "<div class='table-responsive'>";
                        echo "<table class='table  table-hover'>";
                        echo "<thead class='thead-dark'>";
                        echo "<tr class='bg-dark' style='color:White;'><th scope='col'>Image</th>";
                        echo "<th scope='col'>Title</th>";
                        echo "<th scope='col'>Author</th>";
                        echo "<th scope='col'>Publisher</th>";
                        echo "<th scope='col'>Borrower</th>";
                        ...
                        while ($row = mysqli_fetch_array($res_data)) {
                            if ($row['bName'] == "Borrowing") {
                                ...
                                echo "  <td><button class='btn btn-primary'><a style=text-decoration:none;color:White; href=return_book.php?borrowID=".$row['borrowID'].">Return</a></button></td>";
                                echo "  </tr>";
                                echo "</tbody>";
                            } else {
                                $imageURL = 'assets/img/book/' . $row['bookIMG'];
